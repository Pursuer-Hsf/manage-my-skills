...
目的はすべての skill ディレクトリを複製することではありません。各マシンを同じ「必要な skills」の状態へ安全に到達させることです。
...
公開リポジトリにはマネージャーを置きます。非公開リポジトリには個人 skills と移植可能な公開ソース一覧を置き、マシン固有パスと認証情報はローカルだけに保存します。この制御面の分離により、マネージャーは個人 skills を無断で変更せずに独立更新できます。

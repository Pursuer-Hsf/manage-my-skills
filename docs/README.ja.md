...
ローカルの非公開 skill に変更があるか、非公開リポジトリに新しい skill 更新があれば、同期プレビューを自動表示してください。
承認後に変更してください。
```
...
各マシンでマネージャー、ローカル登録、個人 link、公開ソースを確認してください。非公開ライブラリの同期後、欠落または不正な個人 link があれば restore を先にプレビューしてください。
...
個人 skills は非公開ライブラリから復元し、公開 skills は記録されたソースから再導入・更新してください。非公開ライブラリにある skill を公開インストーラーで導入しないでください。

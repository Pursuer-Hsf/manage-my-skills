...
**Agent ファースト、安全性重視の個人用 Agent Skills マネージャー。**

マネージャーは公開、個人の skills は非公開。両者を独立して更新できます。
...
`manage-my-skills` は、AI Agent がユーザー所有の skills を検出、分類、バックアップ、同期、復元するためのオープンソース Agent Skill です。個人の skills は**別の非公開 GitHub リポジトリ**に保存し、変更操作の前には安全確認を行います。

これは skill マーケットプレイスや一括インストーラーではありません。自分で所有・保守する skills のライフサイクルを管理するためのガバナンス層です。

## このリポジトリを Agent に渡すだけで開始

Git コマンドを覚える必要はありません。ファイルシステムと GitHub にアクセスできる Agent にリポジトリ URL を渡し、次のように依頼します。

> このリポジトリの `skills/manage-my-skills/SKILL.md` を読んでください。ローカルの skills をスキャンし、ユーザー所有の skills を別の非公開 GitHub リポジトリで管理してください。変更は必ず先にプレビューし、ブラウザログイン、MFA、承認が必要になったら知らせてください。

Agent は環境確認、GitHub CLI の設定、非公開リポジトリの検証、機密情報のスキャン、インストール、検証を担当します。ユーザーが行うのは、明示された変更の承認とブラウザでの GitHub 認証だけです。

...
目标不是把所有 skill 目录复制到每台机器，而是让每台机器安全地达到同一份“想拥有的 skills”状态。GitHub 是共同中枢，机器之间不需要 SSH 互通。
...
Agent 应负责环境检查、GitHub 访问、私库验证、敏感内容扫描、安装、同步和验证。它会先检查 GitHub 是否可达；如果不可达，会提示使用代理或申请网络访问，再判断登录状态。
...
flowchart LR
    ...
    B -->|"个人内容 + 来源 + 机器清单"| D["私有仓库：my-skills"]
...
公共仓库存放管理器。私有仓库存放个人 skills、可移植的开源来源清单，以及可选的安全机器名称。机器路径、凭据和连接细节只保留在本机。每台机器独立访问 GitHub，不需要服务器之间互相连接。这种控制面分离让管理器可以独立更新，而不会静默改动个人 skills。
...
- 在第一台机器创建私库，再通过 GitHub 独立接入其他机器。
- 经过敏感内容与越界符号链接检查后导入单个自有 skill。
- 接管已有个人 skill 时，先保留本地备份，再替换为受管理链接。
...
- 以一个私库作为唯一事实来源，同步本机和多台服务器；每台机器仍独立保存身份、路径和 GitHub 身份验证。
...
注册这台机器，但未经我确认不要替换或导入任何已有 skill。
先预览，需要登录或确认时告诉我。
```
...
```text
在这台机器配置 manage-my-skills，并接入 OWNER/my-skills。
将它注册为 worker-a，再预览恢复个人 skills 和协调开源 skills 的计划。
这台机器可能只能访问 GitHub，不要假设它能 SSH 到其他机器。
```
...
如果本地私有 skill 发生变化，或私库有新的 skill 更新，自动展示同步预览。
我确认后再修改。
```
...
以私库为共同中枢，不要假设服务器之间可以互相连接。
在每台机器检查管理器、本机注册、个人链接和开源来源。私库同步后，对缺失或错误的个人链接先生成 restore 预览。
先预览每项修复，不要覆盖，完成后汇总各机器本地结果。
```

### 在新机器恢复

```text
将这台机器接入 用户名/my-skills，再恢复全部受管理 skills。
从私库恢复个人 skills，并按清单来源重新安装或更新开源 skills。私库已有的 skill 不要再用公共安装器安装。
先检查冲突，不要覆盖已有内容。
```
...
`library.json` 保存来源管理 skills 的可移植清单，只记录标识、路径和可选版本，不保存可执行安装命令。可选的 `fleet.json` 只保存随机机器 ID、安全名称、角色和启用状态，不保存主机名、IP、SSH 别名、路径或凭据。
...
其中只能包含仓库标识、本机路径、schema 版本、时间戳，以及本机随机 ID 和安全名称，不能包含凭据。

## 安全模型

- 修改前必须预览，并获得明确批准。
- skills 仓库必须验证为 Private。
- 已有本地 checkout 必须指向这个已验证的仓库。
- 凭据由 GitHub 或操作系统凭据管理器保存。
- 疑似敏感内容、危险链接、冲突和已有目标都会让流程停止。
- 管理器不会覆盖、强制推送或自动合并。
- 公共管理器只允许快进更新；存在本地修改或历史分叉时停止。
- 开源、市场、插件和系统内置 skills 保留原更新来源，只同步可移植的来源信息。
- 机器清单不保存连接细节；除非单独配置报告通道，本机检查只能说明当前机器状态。

自动检查只能降低风险，不能证明内容绝对安全。完整策略见 [SECURITY.md](../SECURITY.md)。

## 兼容性与范围

`manage-my-skills` 使用开放的 [`SKILL.md` 格式](https://agentskills.io/)，优先支持 Codex。其他能够访问文件系统的 Agent，在具备 Python 3.9+、Git 和 GitHub 访问权限时也可以使用。每台机器需要自己的 GitHub 访问能力；SSH 不是同步的必需条件。不同 Agent 的发现路径和自动行为可能不同。

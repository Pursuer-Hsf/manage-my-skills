...
**面向 Agent、以安全为先的个人 Agent Skills 管理器。**

管理器保持开源，个人 skills 保持私有，二者独立更新。
...
`manage-my-skills` 是一个开源 Agent Skill，帮助 AI Agent 发现、归类、备份、同步和恢复用户自己维护的 skills。个人 skills 存放在**另一个私有 GitHub 仓库**中，所有修改操作之前都经过安全检查。

它不是 skill 市场，也不是批量安装器。它管理的是属于你自己的 skills 生命周期。

## 把仓库交给 Agent 即可开始

你不需要掌握 Git 命令。把本仓库 URL 发给能够访问本机文件和 GitHub 的 Agent，然后告诉它：

> 阅读本仓库中的 `skills/manage-my-skills/SKILL.md`。扫描我的本地 skills，帮助我使用一个独立的私有 GitHub 仓库管理自有 skills。所有修改必须先预览；需要浏览器登录、MFA 或授权时及时告诉我。

Agent 应负责环境检查、GitHub CLI 配置、私库验证、敏感内容扫描、安装和验证。用户只需要批准明确说明的修改，并在浏览器中完成 GitHub 身份验证。
...
- 支持多机器，每台机器独立保存本机状态和 GitHub 身份验证。

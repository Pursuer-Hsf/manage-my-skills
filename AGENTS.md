...
- Never weaken private-repository verification, secret scanning, staged-path allowlisting, adoption backup, or no-overwrite restore behavior.
...
- Keep `fleet.json` portable and minimal: random UUIDs, safe labels, roles, and enabled state only. Do not add connection details, paths, credentials, or live machine reports.
